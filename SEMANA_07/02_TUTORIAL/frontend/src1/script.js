function calculoNota(){
    ...
    if(nota>=0 && nota<=5){
        document.getElementById('saida').innerHTML = ("Nota armazenada com sucesso!")
...
function TestGETDB(){
    ...
    //$(getDBResDiv).append(resposta[0].title + "<br>" + resposta[1].title + "<br>" + resposta[2].title + "<br> <br>");
    console.log(xhttp.responseText);
}
var user = {

    insert() {
        var title = prompt('Digite uma nota:');
        if (title>=0&&title<=5) {
            if (title.trim() != '') {
                $.ajax({
                    type: 'POST',
